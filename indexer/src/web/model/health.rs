use vecno_rpc_core::GetServerInfoResponse;
...
impl From<(HealthStatus, String)> for HealthVecnod {
    fn from(status_error: (HealthStatus, String)) -> Self {
        let (status, error) = status_error;
        HealthVecnod { status, server_version: None, network_id: None, virtual_daa_score: None, is_synced: None, error: Some(error) }
    }
}

impl From<GetServerInfoResponse> for HealthVecnod {
    fn from(get_server_info_response: GetServerInfoResponse) -> Self {
        ...
        HealthVecnod {
            ...
            error: if !is_synced { Some("Vecnod is not synced".to_string()) } else { None },
        }
    }
}
