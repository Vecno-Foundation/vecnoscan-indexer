...
pub struct TransactionData {
    ...
    pub block_hash: VecnoHash,
    pub block_timestamp: u64,
    pub block_daa_score: u64,
    pub block_blue_score: u64,
}

pub struct VecnoBlocksFetcher {
    ...
    synced: bool,
    lag_count: i32,
    tip_hashes: HashSet<VecnoHash>,
    block_cache: Cache<VecnoHash, ()>,
}

impl VecnoBlocksFetcher {
    ...
    pub fn new(
        ...
        let block_cache: Cache<VecnoHash, ()> =
    ...
    pub async fn start(&mut self) {
        ...
        while !self.signal_handler.is_shutdown() {
            ...
            match self.vecnod_pool.get().await {
                Ok(vecnod) => match vecnod.get_blocks(Some(self.low_hash), true, !self.disable_transaction_processing).await {
                    Ok(response) => {
                            ...
                            && Instant::now().duration_since(self.last_sync_check) >= Self::SYNC_CHECK_INTERVAL
                        {
                            ...
                            if let Ok(block_dag_info) = vecnod.get_block_dag_info().await {
