...
use vecno_rpc_core::{GetBlocksResponse, RpcBlock, RpcTransaction};
use log::info;
    ...
    async fn fetch_blocks(&mut self) {
        ...
        while self.run.load(Ordering::Relaxed) {
            ...
            match self.vecnod_pool.get().await {
                Ok(vecnod) => match vecnod.get_blocks(Some(self.low_hash), true, true).await {
                    Ok(response) => {
