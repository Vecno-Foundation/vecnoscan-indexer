...
use vecno_indexer_cli::cli_args::{CliDisable, CliEnable};
...
pub async fn process_virtual_chain(
    ...
    // Choose a safe default: only consider accepted txs that have at least 6 confirmations
    // This prevents re-indexing due to very shallow reorgs
    let min_confirmation_count = Some(6u64);
    ...
    while !signal_handler.is_shutdown() {
        ...
        match vecnod_pool.get().await {
            Ok(vecnod) => {
                match vecnod
                    .get_virtual_chain_from_block(
                        start_hash,
                        !disable_transaction_acceptance,
                    ...
                    .await
                {
                    Ok(res) => {
                        ...
                        if added_blocks_count > tip_distance {
                            ...
                            let last_accepting_block = vecnod
                                .get_block(*added_chain_block_hashes.last().unwrap(), false)
                                .await
                                .unwrap();
                            ...
                            // Wait for block processor to catch up (prevent too far ahead)
                            loop {
                                if let Some(b) = &metrics.read().await.components.block_processor.last_block {
                                    if checkpoint_block.daa_score.saturating_sub(b.daa_score)
                            ...
                            if !disable_transaction_acceptance {
                                let accepted_transaction_ids =
                                    &res.accepted_transaction_ids[..added_blocks_count - tip_distance];
                                let rows_added = accept_transactions(
                                    batch_scale,
                                    batch_concurrency,
                            ...
                            // Dynamic tip distance logic (unchanged)
                            if dynamic_tip_distance {
                                ...
                                if reorgs_count >= 3 {
                                    tip_distance += 1;
                                    tip_distance_timestamp = SystemTime::now()
                            ...
                            // Update metrics
                            {
                                ...
                                metrics
                                    .components
                                    .virtual_chain_processor
                                    .update_last_block(checkpoint_block.clone().into());
                                ...
                                metrics.components.virtual_chain_processor.tip_distance_date_time =
                                    dynamic_tip_distance.then_some(DateTime::from_timestamp_millis(tip_distance_timestamp as i64)).flatten();
                            }
            ...
            Err(e) => {
                error!("Failed getting vecnod connection from pool: {}", e);
                sleep(err_delay).await;
            }
        }
    }
