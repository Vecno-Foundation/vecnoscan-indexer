...
pub async fn process_virtual_chain(
    ...
    while run.load(Ordering::Relaxed) {
        ...
        match vecnod_pool.get().await {
            Ok(vecnod) => {
                match vecnod.get_virtual_chain_from_block(start_hash, true).await {
                    Ok(res) => {
                        ...
                        if !res.accepted_transaction_ids.is_empty() {
                            ...
                            let timestamp = vecnod.get_block(last_accepting, false).await.expect("GetBlock failed").header.timestamp;
                            update_txs(
                                batch_scale,
