...
```shell
cargo run -- -s ws://<vecnod_host>:7110 -d postgres://postgres:postgres@<postgres_host>:5432
```
...
If you are using vecno-rest-server you can apply the PREV_OUT_RESOLVED=true env var afterward to disable the expensive join for resolve_previous_outpoints=light queries.
