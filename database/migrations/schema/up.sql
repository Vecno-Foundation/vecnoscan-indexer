...
-- NEW: Balances table (live address balances in sompi)
CREATE TABLE balances
(
    script_public_key_address VARCHAR(100) PRIMARY KEY,
    balance                   BIGINT      NOT NULL DEFAULT 0
);

-- Essential indexes for performance
CREATE INDEX idx_balances_balance_desc ON balances (balance DESC);
CREATE INDEX idx_balances_balance_asc  ON balances (balance ASC);
CREATE INDEX idx_balances_address      ON balances (script_public_key_address);

CREATE INDEX idx_balances_nonzero ON balances (balance DESC) WHERE balance > 0;